...
inline uint16_t idx(uint8_t x, uint8_t y) { return static_cast<uint16_t>(y) * WIDTH + x; }

// Serpentine index: odd rows run right→left; delegates to idx() for the base calculation.
inline uint16_t serpentineIdx(uint8_t x, uint8_t y) { return idx(y & 1 ? (WIDTH - 1 - x) : x, y); }
 ...
 public:
  ...
  void loop() override {
    ...
    for (uint8_t y = 0; y < HEIGHT; ++y)  // cache-friendly: 1.5 KB fits in L1; even rows write sequentially, odd rows reversed but within 2 hot cache lines
      for (uint8_t x = 0; x < WIDTH; ++x) physLeds[serpentineIdx(x, y)] = leds[idx(x, y)];
    FastLED.show();
  }
